    ...
    emoticons: [
      {
        ...
        emoji: "🔥",
        label: "Fuego",
        meaning: "Tengo el contacto directo para abrir esa puerta.",
      },
      {
        id: "clap",
        emoji: "👏",
        label: "Aplausos",
        meaning:
          "Conozco a un aliado o proveedor que nos puede acercar estratégicamente.",
      },
      {
        id: "idea",
        emoji: "💡",
        label: "Foco",
        meaning:
          "Tengo una estrategia, consejo legal o solución operativa para resolverlo.",
      },
      {
        id: "bridge",
        emoji: "🤝",
        label: "Puente",
        meaning:
          "Puedo presentar o facilitar una conexión entre las partes clave.",
      },
      {
        id: "build",
        emoji: "🛠️",
        label: "Ejecución",
        meaning: "Tengo recursos, equipo o capacidad para hacerlo realidad.",
      },
    ],
  },
  {
    ...
    emoticons: [
      {
        ...
        emoji: "✅",
        label: "Claro",
        meaning: "Lo puedo aplicar esta semana con un cliente.",
      },
      {
        id: "ejemplo",
        emoji: "🎯",
        label: "Ejemplo",
        meaning: "Necesito un caso real o un 1 a 1 para aterrizarlo.",
      },
      {
        id: "equipo",
        emoji: "🧩",
        label: "Equipo",
        meaning: "Esto le sirve a un aliado de mi Power Team.",
      },
      {
        id: "invitar",
        emoji: "👋",
        label: "Invitar",
        meaning: "Esto es motivo para volver o invitar a alguien.",
      },
      {
        id: "duda",
        emoji: "❓",
        label: "Duda",
        meaning: "Tengo una objeción o un hueco; quiero preguntar.",
      },
    ],
  },
  {
    ...
    emoticons: [
      {
        ...
        emoji: "🙋",
        label: "Yo lo hago",
        meaning: "Puedo cumplir este ask o dar el referido esta semana.",
      },
      {
        id: "unoauno",
        emoji: "☕",
        label: "1 a 1",
        meaning: "Quiero un uno a uno para aterrizar cómo ayudar.",
      },
      {
        id: "presento",
        emoji: "📞",
        label: "Presento",
        meaning: "Puedo hacer una introducción puntual.",
      },
      {
        id: "ahora-no",
        emoji: "⏳",
        label: "Ahora no",
        meaning: "No aplica para mí en este momento.",
      },
    ],
  },
];
...
function currentEmoticons() {
  if (state.question.emoticons && state.question.emoticons.length) {
    return state.question.emoticons;
  }
  return PRESETS[0].emoticons;
}

function emptyCounts(emoticons) {
  return Object.fromEntries(emoticons.map((item) => [item.id, 0]));
}

function publicState() {
  const emoticons = currentEmoticons();
  const counts = emptyCounts(emoticons);
  const voters = Object.fromEntries(emoticons.map((item) => [item.id, []]));

  for (const vote of state.votes.values()) {
    if (!(vote.emoticonId in counts)) continue;
    counts[vote.emoticonId] += 1;
    voters[vote.emoticonId].push({
      name: vote.name,
      at: vote.at,
    });
  }
...
io.on("connection", (socket) => {
  ...
  socket.on("join", ({ name }) => {
    const clean = String(name || "")
      ...
      .slice(0, 32);
    if (clean.length < 2) {
      socket.emit("error-msg", "Escribe un nombre de al menos 2 caracteres.");
      return;
    }
    ...
    if (existing) {
      socket.emit("error-msg", "Ese nombre ya está en uso. Prueba con otro.");
      return;
    }
  ...
  socket.on("vote", ({ emoticonId }) => {
    ...
    if (!participant) {
      socket.emit("error-msg", "Entra primero con tu nombre.");
      return;
    }
    if (!state.question.active) {
      socket.emit("error-msg", "No hay una votación abierta.");
      return;
    }
    const emoticons = currentEmoticons();
    const chosen = emoticons.find((item) => item.id === emoticonId);
    if (!chosen) {
      socket.emit("error-msg", "Emoticón no válido para esta pregunta.");
      return;
    }

    state.votes.set(participant.name.toLowerCase(), {
      emoticonId,
  ...
  socket.on("admin:auth", ({ pin }) => {
    if (String(pin) !== ADMIN_PIN) {
      socket.emit("admin:auth-result", { ok: false });
      return;
    }
  ...
  socket.on("admin:set-question", ({ text, emoticons, presetId }) => {
    ...
    if (clean.length < 4) {
      socket.emit("error-msg", "La pregunta debe tener al menos 4 caracteres.");
      return;
    }
    ...
    if (!cleanedEmos) {
      socket.emit(
        "error-msg",
        "Revisa los emoticones: mínimo 2, cada uno con emoji, nombre y significado."
      );
      return;
    }
    const knownPreset = PRESETS.some((preset) => preset.id === presetId)
      ? presetId
      : "custom";
    state.question = {
      ...
      text: clean,
      active: true,
      emoticons: cleanedEmos,
      presetId: knownPreset,
    };
    state.votes.clear();
    broadcast();
  });
  ...
  socket.on("admin:toggle", () => {
    ...
    state.question.active = !state.question.active;
    broadcast();
  });
